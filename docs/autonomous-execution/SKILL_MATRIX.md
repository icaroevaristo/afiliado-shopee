...
| --- | --- | --- | --- | --- | --- |
| R1 runtime canônico | runtime/data | goal-guard, safe-command, git, backend, SQL, QA, ship | observability | iniciar outra worktree/volume; cleanup amplo | identidade/volume/restart comprovados |
...
| R8 SEND controlado | runtime/data/security | todas as hard guards, backend, SQL, QA, secrets, observability, ship | API reviewer | provider sem autorização; retry incerto | autorização, budget, receipt e lifecycle |
| R9 ativação diária | orchestrator + owner | goal-guard, safe-command, secrets, observability, ship | reviewer | remover pause automaticamente | decisão humana registrada |
