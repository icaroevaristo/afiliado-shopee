    ...
    <div className="grid gap-6">
      ...
      <article className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="flex flex-col gap-5 sm:flex-row">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-slate-600">
              {detail.shopName} · {detail.source}
            </p>
            <p className="mt-2 text-sm text-slate-600">
              Categorias:{' '}
              {categoryLabels(detail.categoryIds, categories).join(', ') || '—'}
            </p>
            <p className="mt-2 text-sm text-slate-600">
        ...
        <dl className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Metric
            label="Preço atual"
            value={formatCurrency(Number(detail.price))}
          />
          <Metric
            label="Preço de referência"
            value="Não informado pelo provider"
          />
          <Metric
            label="Link afiliado"
          ...
          <Metric
            label="Comissão"
            value={formatPercent(detail.commissionRate)}
          />
          <Metric
            label="Score comercial atual"
            value={formatNumber(detail.bestCurrentCommercialScore)}
          />
          <Metric
            label="Candidates atuais"
            value={formatNumber(
              detail.commercialStateSummary.currentCandidateCount,
            )}
          />
          <Metric
            label="Na fila"
            value={formatNumber(detail.commercialStateSummary.queued)}
          />
          <Metric
            label="Copy pronta"
            value={formatNumber(detail.commercialStateSummary.copyReady)}
          />
          ...
          <Metric
            label="Destinos enviados"
            value={formatNumber(detail.sentDestinationCount)}
          />
          <Metric
            label="Último envio"
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Somente candidates coerentes com o snapshot comercial atual entram no
          score.
        </p>
        {detail.commercialScores.length === 0 ? (
          <p className="mt-4 text-sm text-slate-500">
            Nenhum candidate atual coerente.
          </p>
        ) : (
            ...
            {detail.commercialScores.map((score) => (
              ...
              >
                ...
                <p className="text-slate-600">
                  Score {score.score} · {score.candidateStatus} · nicho{' '}
                  {score.nicheId}
                </p>
              </li>
            ))}
      ...
      >
        {detail.dispatchHistory.items.map((dispatch) => (
          ...
          >
            <p className="font-medium text-slate-900">
              {dispatch.destination.name} · {dispatch.status}
            </p>
            <p className="text-slate-600">
              {dispatch.destination.type} · instância{' '}
              {dispatch.instanceName ?? 'legada/não registrada'} ·{' '}
              {formatDateTime(dispatch.sentAt)}
            </p>
            <p className="text-slate-600">
              Tentativas: {dispatch.attemptCount}
              {dispatch.run
                ? ` · final ${dispatch.run.finalStatus ?? '—'}`
                : ''}
            </p>
          </li>
        ))}
        ...
        {detail.snapshotHistory.items.map((snapshot) => (
          ...
          >
            <p className="font-medium text-slate-900">
              ...
              {formatCurrency(Number(snapshot.price))}
            </p>
            <p className="text-slate-600">
              Desconto {formatPercent(snapshot.discountRate)} · comissão{' '}
              {formatPercent(snapshot.commissionRate)} · capturado{' '}
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Nao foi possivel carregar as opcoes manuais.',
      );
    } finally {
      setLoading(false);
    }
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Nao foi possivel criar a publicacao manual.',
      );
    } finally {
  ...
  return (
    <section className="rounded-lg border border-amber-200 bg-amber-50/40 p-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-950">
            Publicacao manual segura
          </h2>
          <p className="mt-1 text-sm text-slate-700">
            Selecione exatamente 1 grupo. A mesma copy e o mesmo pipeline
            comercial serao usados.
          </p>
        </div>
      ...
      {options ? (
        <>
            ...
            <Metric
              ...
              value={
                options.product.snapshot
                  ? `Revisão ${options.product.snapshot.revision} · ${options.product.snapshot.fingerprint.slice(0, 12)}...`
                  : 'Ausente'
              }
            />
            <Metric
              label="Produto elegivel"
              value={options.product.available ? 'Sim' : 'Nao'}
            />
            <Metric
              label="Grupo selecionado"
              value={selectedId ? '1/1' : '0/1'}
            />
          </div>
          ...
          <div className="mt-5 rounded border border-slate-200 bg-white p-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Confira o produto, o snapshot atual e cada grupo. A copy não é
              editável aqui e seguirá o mesmo pipeline comercial.
            </p>
            {!selectedId ? (
              <p className="mt-3 text-sm text-slate-500">
                Selecione pelo menos um grupo para revisar a intenção.
              </p>
            ) : (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="text-slate-600">
                          {group.eligible ? 'Elegível' : 'Bloqueado'}
                        </span>
                      </div>
                      {group.draftPreview ? (
                        <details className="mt-2">
                          <summary className="cursor-pointer font-medium text-slate-700">
                            Ver prévia da copy pronta
                          </summary>
                          <div className="mt-2 grid gap-1 rounded bg-slate-50 p-3 text-slate-700">
                            <p className="font-medium">
                              {group.draftPreview.title}
                            </p>
                            <p className="whitespace-pre-wrap">
                              {group.draftPreview.message}
                            </p>
                        ...
                        <p className="mt-2 text-xs text-slate-600">
                          Copy ainda não pronta; será preparada pelo mesmo
                          pipeline após a confirmação.
                        </p>
                      )}
                    </li>
                  );
          ...
          <div className="mt-5 grid gap-2">
            ...
            {options.groups.length === 0 ? (
              <p className="text-sm text-slate-600">
                Nenhum grupo disponivel para selecao.
              </p>
            ) : null}
            {options.groups.map((group) => {
              ...
              return (
                <label
                  ...
                  className={`flex cursor-pointer items-start gap-3 rounded border p-3 ${selected ? 'border-amber-400 bg-white' : 'border-slate-200 bg-white'} ${group.eligible ? '' : 'cursor-not-allowed opacity-70'}`}
                >
                  <input
                    type="radio"
                  ...
                  <span className="min-w-0 text-sm">
                    ...
                    <span className="block text-slate-600">
                      {group.eligible
                        ? `Copy ${copyStatusLabel(group.copyStatus)}`
          ...
          {!request ? (
            <div className="mt-5 grid gap-3 rounded border border-slate-200 bg-white p-4">
              ...
              <p className="text-xs text-slate-600">
                A acao fica vinculada a uma chave de idempotencia desta sessao e
                nao cria retry automatico.
              </p>
            </div>
          ) : (
            <div className="mt-5 rounded border border-slate-200 bg-white p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="text-sm font-semibold text-slate-900">
                    Request: {manualRequestStatusLabel(request.status)}
                  </p>
              ...
              <ul className="mt-3 grid gap-2 text-sm">
                {request.targets.map((target) => (
                  <li
                    key={target.id}
                    className="flex flex-wrap justify-between gap-2 rounded border border-slate-200 p-2"
                  >
                    <span>
                      {target.destination?.name ?? target.destinationId}
                    </span>
              ...
              {request.status === 'AMBIGUOUS' ? (
                <p className="mt-3 rounded border border-red-200 bg-red-50 p-3 text-xs text-red-800">
                  Há um resultado ambíguo. Não tente reenviar; faça a
...
function categoryLabels(categoryIds: string[], categories: ShopeeCategory[]) {
  const labels = new Map(
    categories.map((category) => [category.id, category.displayLabel]),
  );
  return categoryIds.map(
    (categoryId) => labels.get(categoryId) ?? `Categoria ${categoryId}`,
  );
}
...
function HistorySection({
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5">
      ...
      <p className="mt-3 text-xs text-slate-500">Página {page}</p>
    </section>
  );
}
