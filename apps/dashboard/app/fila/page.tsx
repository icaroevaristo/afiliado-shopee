...
export default function QueuePage() {
  ...
  useEffect(() => {
    ...
    }).catch((cause) => { if (active) setError(cause instanceof Error ? cause.message : 'Nao foi possivel carregar campanhas.'); }).finally(() => { if (active) setLoadingCampaigns(false); });
    return () => { active = false; };
  }, []);
  ...
  useEffect(() => {
    ...
    setError(null);
    void listCommercialCampaignQueue(campaignId, status ? { status } : {}).then((response) => { if (active) setItems(response.items); }).catch((cause) => { if (active) setError(cause instanceof Error ? cause.message : 'Nao foi possivel carregar a fila.'); }).finally(() => { if (active) setLoadingQueue(false); });
    return () => { active = false; };
  }, [campaignId, status]);
  ...
  return (
    <>
      <OpsPageHeading eyebrow="Candidate queue" title="Fila" description="Candidatos ordenados pelo ranking persistido da campanha. Nenhuma mineracao ou geracao e disparada nesta tela." />
      <OpsSection title="Fila da campanha" meta={selectedCampaign ? `${selectedCampaign.name} · alvo ${selectedCampaign.queueTargetSize}` : 'Selecione uma campanha ativa.'} actions={campaigns.length > 0 ? <select className="ops-button" value={campaignId} onChange={(event) => setCampaignId(event.target.value)} aria-label="Selecionar campanha"><option value="">Selecionar campanha</option>{campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}{campaign.active ? '' : ' · inativa'}</option>)}</select> : null}>
        ...
        {error ? <OpsState title="Fila indisponivel" message={error} tone="danger" /> : null}
        {!loadingQueue && !loadingCampaigns && !error && !campaignId ? <OpsEmpty title="Nenhuma campanha disponivel" message="A API nao retornou campanha para consultar a fila." /> : null}
