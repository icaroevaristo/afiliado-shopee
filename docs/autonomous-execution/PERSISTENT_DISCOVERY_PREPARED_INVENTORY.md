...
revision. Offer expiry or snapshot/copy drift terminalizes the candidate with
the corresponding reason. Reservation leases are recovered by reading the
...
price, category, keyword, and minimum-score rules. For an ordered A/B
assignment, the supervisor preloads protected candidate identities for the
complete route before filling either instance; the slot keeps its persisted
instance and assignment revision.
...
100 logical slots. Operational infrastructure and external providers remain
